...
public class TestWorker {
    ...
    public void createWorker() {
        log.info("Registering TestWorker..");
        var workerFactory = WorkerFactory.newInstance(workflowClient, defaultWorkerFactoryOptions);
        var worker = workerFactory.newWorker(AllFunction.TEST.name(), defaultWorkerOptions);

        var completionClient = workflowClient.newActivityCompletionClient();
        worker.registerWorkflowImplementationTypes(defaultWorkflowImplementationOptions, TestWorkflowImpl.class);
